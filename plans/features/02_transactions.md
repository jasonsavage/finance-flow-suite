...
- **`POST /transactions/upload?bankAccount=Chase%20Checking`**: 
...
- **`GET /transactions/list?from=YYYY-MM-DD&to=YYYY-MM-DD`**: 
  ...
  **Note**: Skip any transaction rows that contain the word "pending" in the description or any other field. 

  **Step 1**: Generate a unique ID for each transaction by creating a hash of the row data and the `user_id`. Use it to populate the `id` field. This will allow future deduping of transactions. 
