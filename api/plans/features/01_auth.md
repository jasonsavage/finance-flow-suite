...
- **`POST /auth/register`**: 
  ...
  - **Logic**: Securely hashes the password with `bcrypt`, generates an `account_id`, copies `username` to the `email` field, and saves the new user to the `users` database table using the repository pattern. Returns a session token upon success.
- **`POST /auth/login`**: 
  - **Payload**: Requires `username` and `password`.
