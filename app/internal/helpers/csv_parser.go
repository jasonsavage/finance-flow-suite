...
func ParseTransactionsCSV(r io.Reader, userID string, bankAccountName string) ([]models.Transaction, error) {
	...
	for {
		...
		// Skip any pending transactions securely
		rowStr := strings.Join(record, ",")
		if strings.Contains(strings.ToUpper(rowStr), "PENDING") {
			continue
		}
		hash := sha256.Sum256([]byte(rowStr + userID))
